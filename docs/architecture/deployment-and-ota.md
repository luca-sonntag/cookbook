...
* **Zentrale Git-Utilities (`frontend/scripts/git-utils.ps1`):** Bündelt wiederverwendbare Git-Hilfsfunktionen (`Assert-GitClean`, `Invoke-GitMasterMergeAndTag`). Alle Release-Skripte (`deploy.ps1`, `deploy-playstore.ps1`, `deploy-ota.ps1`) nutzen dieses Modul für Pre-Flight-Checks, automatische Merges von `develop` nach `master` sowie das Tagging & Pushing.
...
3. **Play-Store-Release Reset:** `resetWhenUpdate: true` löscht alle gecachten OTA-Bundles auf dem Gerät bei jedem Play-Store-App-Update.
