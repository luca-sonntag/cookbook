...
export default function App() {
  ...
  const copyRecipeMarkdown = () => {
    const activeRecipe = activeView === 'history' ? selectedJob?.recipe : recipe;
    if (!activeRecipe) return;

    let md = `# ${activeRecipe.title}\n\n${activeRecipe.description}\n\n`;
    md += `**Prep Time:** ${activeRecipe.prepTime} | **Cook Time:** ${activeRecipe.cookTime} | **Servings:** ${activeRecipe.servings}\n\n`;
    
    md += `## Ingredients\n`;
    activeRecipe.ingredients.forEach((ing: Ingredient) => {
    ...
    md += `## Instructions\n`;
    activeRecipe.instructions.forEach((step: InstructionStep) => {
      md += `${step.step}. ${step.description}\n`;
    });
    md += `\n`;

    if (activeRecipe.equipment && activeRecipe.equipment.length > 0) {
      md += `## Equipment\n`;
      activeRecipe.equipment.forEach((item: string) => {
        md += `- ${item}\n`;
      });
      md += `\n`;
    }

    if (activeRecipe.tips && activeRecipe.tips.length > 0) {
      md += `## Chef Tips\n`;
      activeRecipe.tips.forEach((tip: string) => {
        md += `- ${tip}\n`;
      });
      md += `\n`;
    }
          ...
          <div className="flex flex-col gap-4">
            {selectedJob ? (
              ...
              <div className="flex flex-col gap-4">
                ...
                {renderRecipe(selectedJob.recipe)}
              </div>
            ) : (
