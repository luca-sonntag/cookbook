...
export default function ShoppingList({
  ...
  const suggestions = ['Stück', 'g', 'ml', 'Pkg.', 'Dose', 'TL', 'EL'];
      ...
      <Card className="glass-panel p-5 rounded-2xl border border-black/5 dark:border-white/5">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-1.5 items-center mt-1">
            <span className="text-[10px] text-gray-500 dark:text-gray-400 mr-1">Vorschläge:</span>
            {suggestions.map((sug) => (
              <button
                key={sug}
                type="button"
      ...
      <Card className="glass-panel p-5 rounded-2xl border border-black/5 dark:border-white/5">
        ...
        {totalCount === 0 ? (
          <div className="text-center py-8 flex flex-col items-center justify-center">
            ...
            <h4 className="text-sm font-semibold text-gray-900 dark:text-white">Deine Einkaufsliste ist leer</h4>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 max-w-xs leading-normal">
              Füge Zutaten direkt aus einem Rezept hinzu oder trage eigene Einträge oben ein!
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-6">
            ...
            {aggregatedList.unchecked.length > 0 && (
              <div className="flex flex-col gap-2.5">
                <h4 className="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  Noch zu kaufen ({aggregatedList.unchecked.length})
                </h4>
                <ul className="flex flex-col gap-1.5">
                  {aggregatedList.unchecked.map((item) => {
            ...
            {aggregatedList.checked.length > 0 && (
              <div className="flex flex-col gap-2.5 pt-2 border-t border-black/5 dark:border-white/5">
                <h4 className="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  Bereits im Korb ({aggregatedList.checked.length})
                </h4>
                <ul className="flex flex-col gap-1.5">
