...
export const CookingModeIngredients: React.FC<CookingModeIngredientsProps> = ({
  ...
  return (
    <div className="w-full px-5 py-4 sm:px-6 sm:py-5 border-t border-black/5 dark:border-white/5 flex flex-col text-left">
      ...
      <div className="flex items-center justify-between gap-2 mb-3 shrink-0">
        ...
        <span className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 rounded-full px-2.5 py-0.5 tabular-nums select-none">
          {ingredients.length}
        </span>
      </div>

      {/* 2-Column Grid directly on card surface */}
      <ul className="grid grid-cols-2 gap-x-4 sm:gap-x-6 gap-y-2.5 text-xs">
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center leading-snug">
                ...
                {/* Name & modifier */}
                <div className="flex items-baseline flex-wrap gap-x-1 min-w-0 text-xs sm:text-[13px] font-semibold text-gray-900 dark:text-white">
                  <span className="truncate">{ing.name}</span>
                  {ing.modifier && (
                    <span className="text-[10.5px] text-gray-400 dark:text-gray-500 font-normal truncate">
                      ({ing.modifier})
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 font-normal mt-0.5">
                  {(amountStr || unitStr) && (
                    <span className="font-bold text-emerald-600 dark:text-emerald-400 tabular-nums truncate">
