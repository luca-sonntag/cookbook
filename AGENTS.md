...
* **Benutzername/Handle-Extraktion & Fallback:** Um fehlerhafte und ungültige Creator-Profillinks zu vermeiden (wenn Scraper wie RapidAPI nur den Anzeigenamen wie "Ashley Markle" statt des technischen Usernames zurückgeben), extrahiert das Backend den technischen Handle direkt aus der URL, falls dieser dort enthalten ist. Das Frontend prüft zudem die Validität des gespeicherten Handles. Entspricht der Handle nicht den Namenskonventionen der Social-Media-Plattform (z. B. bei Leerzeichen), wird statt eines defekten Profillinks direkt die Original-Reel-URL als Linkziel verwendet, damit Nutzer das Original-Video aufrufen und den Creator dort finden können.
