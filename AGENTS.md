...
### UI-Komponenten
* **`PremiumModal.tsx`:** High-End glassmorphischer Upsell-Dialog. Wird über `createPortal(document.body)` gerendert. Bietet eine optische Premium-Aufbereitung mit animiertem Crown-Asset, ambienten Hintergrundbeleuchtungen und dedizierten glassmorphic Cards für die 6 Premium-Features mit farblich hervorgehobenen Icons (`Zap`, `Wand2`, `Activity`, `ShoppingCart`, `Timer`, `BookOpen`). Listet alle Premium-Features auf, löst beim Klick auf CTA `buyPremium()` aus (RevenueCat). Zeigt "Du hast Premium"-State wenn bereits aktiv.
