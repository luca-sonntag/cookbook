...
* **Recipe Copilot (Function Calling / Tool Use):** Ein rezept-spezifischer Chatbot (`POST /api/jobs/:id/chat`), der dem Nutzer Fragen zur Zubereitung oder Zutaten beantwortet. Gemini ist mit Tools (`modify_current_recipe`, `add_missing_ingredients_to_shopping_list`, `set_cooking_timer`) ausgestattet. Die Ausführung wird im Backend verarbeitet (z. B. synchrone Rezept-Mutation via `remixRecipe` und persistente Speicherung in Supabase), an Gemini zurückgemeldet und als finaler Chatbot-Text zusammen mit den Tool-Metadaten an den Client gesendet.
...
* **Rezeptverlauf & Interaktion:**
  ...
  * **Recipe Copilot Chatbot (`RecipeCopilot.tsx`):**
    ...
    * **Verbindung zu Modulen:** Bietet kontextbezogene Quick-Chips für Rezeptanpassungen (Veganisierung, Portionsänderung), Kochfragen und einen Zutatennotfall.
