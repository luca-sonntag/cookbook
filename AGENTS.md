...
* **PWA & Share Target Integration:**
  ...
  * **Native Android Share Intents (Capacitor):** Unterstützt native Teilen-Intents über das `send-intent` Plugin. Nach dem Auslesen der URL wird der Intent nativ mit `SendIntent.finish()` beendet/gelöscht. Dies verhindert, dass derselbe Intent erneut ausgewertet wird, wenn der App-Zustand re-evaluiert wird (z. B. wenn sich das `user`-Objekt durch Einstellungsänderungen aktualisiert). Auf Web-Ebene stellt das Modul-Flag `isWebShareProcessed` sicher, dass der Interceptor nur einmal pro Ladevorgang ausgeführt wird.
...
* **`CatalogEmptyState.tsx`:** Visueller und interaktiver Step-by-Step Onboarding-Guide, der angezeigt wird, wenn das Rezept-Kochbuch leer ist. Führt den Nutzer durch das Entdecken von Videos, Kopieren von Links und Extrahieren im "Neues Rezept" Tab, mit einem prominenten CTA zur Navigation.
