...
* **Aktiver Provider:** **Ultra-Fast Social Media Downloader** (`rover-omniscraper/media-downloader-actor`), aufgerufen mit `downloadMode: 'auto'` (Video+Audio als eine Datei), deckt 21+ Plattformen ab (u. a. Instagram, TikTok, YouTube, Facebook, Twitter/X). Der Actor legt die Mediendatei im Apify **Key-Value-Store** ab; das Dataset-Item liefert `downloadUrl` (direkter Datei-Link) sowie `metadata.title`/`artist`. **Wichtige Einschränkung:** Dieser Actor liefert **keine** Post-Caption und **kein** Thumbnail (`imageUrl` bleibt leer) — für diese Felder greift bei Bedarf der lokale yt-dlp-Fallback. Siehe `docs/apify.social-downloader.md` für das Input/Output-Schema des Actors.
