...
11. **Recipe Remix (KI-Anpassung im Chat):**
   * Der Remix ist in den Copilot-Chat integriert. Die Action-Dock hat einen Chat-Icon-Button (MessageCircle) der den Copilot-Drawer öffnet.
   * Nutzer schreibt einen Remix-Wunsch (z. B. "Mach es vegan") in den Chat. Gemini erkennt die Absicht, führt den Remix aber **noch nicht** aus.
   * Eine amber-farbene Bestätigungskarte erscheint im Chat mit zwei Buttons:
     * **"Aktuelles ersetzen"**: Überschreibt das bestehende Rezept in-place (`updateJob`), Bilder bleiben erhalten. Nutzer bleibt auf der aktuellen Ansicht.
     * **"Als neues Rezept"**: Erstellt einen neuen Remix-Job (`saveCompletedRemix`) mit `parentJobId`.
   * Erst bei Klick auf einen der Buttons wird `POST /api/jobs/:id/chat/confirm` mit `replaceCurrent: true/false` aufgerufen, was `remixRecipe` ausführt und das Ergebnis speichert.
...
| Rezept-Remix (Copilot Chat) | Lock-Badge + öffnet PremiumModal | Aktiviert |
...
### Backend-Gating
* **`POST /api/jobs/:id/chat`:** Prüft den Premium-Status des Nutzers via Supabase Admin API (`auth.admin.getUserById`). Gibt `403 Forbidden` mit `code: 'PREMIUM_REQUIRED'` zurück, wenn `tier !== 'premium'`. Chat und Remix sind damit nur für Premium-User verfügbar.
